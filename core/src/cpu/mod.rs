...
impl Fault {
    pub fn from_uc_error(error: uc_error) -> Result<(), Fault> {
        match error {
            ...
            uc_error::RESOURCE => {
                #[cold]
                fn insufficient() -> Fault {
                    Fault::InsufficientResources
                }
                Err(insufficient())
            }
            uc_error::EXCEPTION => {
                #[cold]
                fn unhandled() -> Fault {
                    Fault::UnhandledException
                }
                Err(unhandled())
            }
            _ => {
                #[cold]
                fn unknown(err: uc_error) -> ! {
                    panic!("unknown execution error `{err}`")
                }
                unknown(error)
            }
        }
...
impl CoreIndex {
    ...
    fn all() -> impl Iterator<Item = Self> {
        (0..8).filter_map(Self::from_index)
    }
...
impl CpuCore {
    fn load_from_reg(&self, reg: RegisterARM64) -> u64 {
        unsafe {
            ...
            load_result
                .and(Ok(value))
                .unwrap_or_else(|_| panic!("failed to load register {reg:?}"))
        }
    }
    ...
    fn store_to_reg(&self, reg: RegisterARM64, value: u64) {
        unsafe {
            ...
            Result::from(store_result)
                .unwrap_or_else(|_| panic!("failed to store register {reg:?}"))
        }
    }
...
impl CpuCore {
    pub fn execute_at_ext(&self, start: u64, count: Option<NonZero<usize>>) -> Result<(), Fault> {
        unsafe {
            let timeout = 0;
            let count = count.map_or(0, |c| c.get());
    ...
    pub fn step(&self) -> Result<(), Fault> {
        let steps = NonZero::new(1).unwrap();
        let pc = self.load_pc();
...
impl Cpu {
    pub fn new() -> Result<Self, CpuInitError> {
        ...
        let mut cores_init: [Option<CpuCore>; CpuCore::COUNT as usize] =
            [const { None }; CpuCore::COUNT as usize];
        ...
        for core in CoreIndex::all() {
            ...
            match unsafe { CpuCore::internal_core(stack_top, shared_memory.as_ref()) } {
                Ok(cpu_core) => {
                    cores_init[idx] = Some(cpu_core);
                    success_count += 1;
                }
                Err(e) => {
                    for core_opt in cores_init.iter_mut().take(success_count) {
                        if let Some(core) = core_opt.take() {
                            drop(core);
                        }
                    }
                    return Err(e);
        ...
        let mut cores = std::mem::MaybeUninit::uninit_array();
        for (i, core_opt) in cores_init.into_iter().enumerate() {
            cores[i].write(core_opt.expect("core should be initialized"));
        }

        Ok(Self {
            cores: unsafe { std::mem::MaybeUninit::array_assume_init(cores) },
            shared_memory,
        })
    }
